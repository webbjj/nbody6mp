...
*        BEGIN NBODY6MP
         ...
         ELSE
           DMX(K) = MLWIND(KW,LUM,RM0,M1,MC,RLPERI,ZPOP(MPTYPE(I)))
         END IF
*        END NBODY6MP
         DMA(K) = 0.D0
*
...
*       Check rejuvenation of MS, HG or HE star.
            IF(KW.LE.2.OR.KW.EQ.7)THEN
               M0 = M1
               IF (I.EQ.1295) THEN
*                 WRITE(*,*) 'DEBUG 1295 BEFORE STAR',M0,M1,
*     &kw,zpars

               END IF
               CALL STAR(KW,M0,M1,TM,TN,TSCLS,LUMS,GB,ZPARS)
               IF (I.EQ.1295) THEN
*                 WRITE(*,*) 'DEBUG 1295 AFTER STAR',M0,M1,
*     &kw,zpars
...
*       Determine stellar evolution time scales and luminosity.
         IF (I.EQ.1295) THEN
*           WRITE(*,*) 'DEBUG 1295 BEFORE STAR2',M0,M1,KW,ZPARS
         END IF
         CALL star(KW,M0,M1,TM,TN,TSCLS,LUMS,GB,ZPARS)
*        DEBUG
         IF(I.EQ.1295) THEN
*           WRITE(*,*) 'DEBUG MDOT CALLSTAR 1295: ',M0,M1,KW,
*     &ZPARS
         END IF
*
